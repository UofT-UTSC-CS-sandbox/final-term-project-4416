...
function NoteBrowser(props) {
    ...
    async function retrieveNotes() {
        try {
            const response = await axios.post("http://localhost:8000/browser", {a: "get notes"});
    ...
    async function searchNotes() {
        try {
            const response = await axios.post("http://localhost:8000/api/searchNotes", { term: searchTerm });
            setNotes(response.data.data);
        } catch (err) {
            console.error(err);
        }
    ...
    async function deleteSelectedNotes() {
        try {
            await axios.post("http://localhost:8000/deleteNotes", Array.from(selectedNotes));
